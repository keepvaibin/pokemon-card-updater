...
async function handler(req: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const apikey = process.env.X_API_KEY_9;
  if (!apikey) {
    return { status: 500, jsonBody: { message: "X_API_KEY_9 env var missing" } };
  }

  console.log(`🧪 worker9 received range ${pageStart}–${pageEnd}`);
...
app.http("pokemon-card-updater-worker9", {
  methods: ["POST"],
  authLevel: "function",
  handler,
});
