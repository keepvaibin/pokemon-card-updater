...
async function handler(req: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const apikey = process.env.X_API_KEY_5;
  if (!apikey) {
    return { status: 500, jsonBody: { message: "X_API_KEY_5 env var missing" } };
  }

  console.log(`🧪 worker5 received range ${pageStart}–${pageEnd}`);
...
app.http("pokemon-card-updater-worker5", {
  methods: ["POST"],
  authLevel: "function",
  handler,
});
