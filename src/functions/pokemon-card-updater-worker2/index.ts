...
async function handler(req: HttpRequest, _context: InvocationContext): Promise<HttpResponseInit> {
  ...
  const apikey = process.env.X_API_KEY_2;
  if (!apikey) {
    return { status: 500, jsonBody: { message: "X_API_KEY_2 env var missing" } };
  }

  console.log(`🧪 worker2 received range ${pageStart}–${pageEnd}`);
...
app.http("pokemon-card-updater-worker2", {
  methods: ["POST"],
  authLevel: "function",
  handler,
});
